...
<html lang="en">
  ...
  <body role="document">
	...
	{
		var chatpage = io.connect('http://localhost:3000/?token=<?php echo $jwt;?>', {
			reconnection: true,
		...
		var chatpage=chatpage.on('connect_failed', function (reason) {
				$('.spinner-feeds').hide();
				$('#chat-window').hide();
				$('.error-feeds').show();
			...
			.on('connect', function () {
				console.log(user_id);
				console.log('sucessfully established a connection of chatpage with the namespace1');
