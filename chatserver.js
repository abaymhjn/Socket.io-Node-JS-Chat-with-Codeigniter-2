...
module.exports.getUserFeeds = function (chatpage, socket, io, pool,async)
{
	...
	socket.on('sendcomment', function (data)
	{
		...
		rooms['room' + data.room_id] = 'room' + data.room_id;
		socket.join('room' + data.room_id);
		//socket.join(userbase[data_from_client.to_user]);
		pool.getConnection(function (err, connection)
        {
				...
				async.parallel([
				...
				], function (err, results)
					{
						...
						if (results[0])
						{
							...
							//io.of(chatpage).to(room);
							//socket.in("room1").emit('showcomment', "You are in room no. 1");
							//socket.broadcast.to('room'+ data.room_id).emit('showcomment',{room_comment: results[0],comment_count: results[1]});
							socket.to('room1').emit('showcomment', {room_comment: results[0],comment_count: results[1]});
							socket.emit('showcomment', {room_comment: results[0],comment_count: results[1]});
							/*socket.broadcast.to('room'+ data.room_id).emit('showcomment',
                            {
